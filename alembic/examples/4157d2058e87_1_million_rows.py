...
def upgrade() -> None:
    ...
    with db.engine.begin() as conn:
        ...
        user_ids = conn.execute(sqlalchemy.text("""
                SELECT id
                FROM users
                """)).scalars().all()
        print(
        ...
        if user_ids:
            ...
            for id in user_ids:
                if i % 1000 == 0:
                    print(i)
                i += 1
                wallet.append({"user_id": id, "change": 100000})
            if wallet:
                conn.execute(
                    sqlalchemy.text("""
        ...
        print(
            "-------------------------------------------Making 100,000 Games and leagues--------------------------------------------------------------"
        )
        ...
        conn.execute(sqlalchemy.text("""
            ...
            ('nhl', 'hockey')
            """))
        teams = conn.execute(sqlalchemy.text("""
                SELECT id
                FROM teams
                """)).scalars().all()

        league_ids = conn.execute(sqlalchemy.text("""
                SELECT id 
                FROM leagues
                """)).scalars().all()
            ...
            games.append(
                {
                    ...
                    "date": fake.date_time_between(
                        start_date="-5y", end_date="now", tzinfo=None
                    ),
        ...
        game_ids = conn.execute(sqlalchemy.text("""
                SELECT id
                FROM games
                """)).scalars().all()
...
def downgrade() -> None:
    ...
    with db.engine.begin() as connection:
        connection.execute(
            sqlalchemy.text("""TRUNCATE TABLE users RESTART IDENTITY CASCADE""")
        )
        connection.execute(
            sqlalchemy.text("""TRUNCATE TABLE teams RESTART IDENTITY CASCADE""")
        )
    pass
